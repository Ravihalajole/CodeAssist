...
- `TransactionManager.kt` — staging across a command block; partial-success tolerant (NOT atomic by design — non-atomic saves tokens). Failed commands surface individually; successful ones still commit so the model re-emits only the failures. Before the first mutating batch of a session it lazily snapshots a one-shot git checkpoint (`AgentOrchestrator.sessionCheckpointRef` via `GitManager.createCheckpoint`) and tags it `codeassist-session-start`; every committed batch is additionally tagged `codeassist-round-<n>` so the "Undo Session" toolbox tool can reset to any round. `AgentPolicy` DENY rules are enforced here as pre-execution blocks (the single execution choke point shared by the loop and the QS-tile path). Feedback includes a per-file `--- FILE STATUS ---`, a `--- BATCH SUMMARY ---`, and a `--- STATE SNAPSHOT ---` (post-write line counts + top-level symbols via `WorkspaceScope.outlineFor`, plus git state via `GitManager.repositorySnapshot`).
- `GitManager.kt` — JGit staging. `commitChanges` stages only modified/deleted paths via per-path `addFilepattern`; `commitAllChanges` does a full add; `createCheckpoint` commits everything (or returns HEAD when clean); `tagCurrentHead`/`listCheckpoints` implement round-granular undo points. Both serialize through an internal `Mutex` and clear `.git/index.lock` deadlocks. All operations resolve the *enclosing* repo by walking up (`findEnclosingRepo`) so a nested workspace reuses the parent repo; subtree staging is scoped to the workspace.
- `agent/` — `AgentOrchestrator`, `AgentState`, `ToolboxManager`, `SessionTranscript`, `AgentPolicy`. `AgentOrchestrator` hard-stops the loop after `MAX_LOOP_ITERATIONS = 25` rounds. It owns a cooperative **Stop token**: `requestStop()` sets a `@Volatile` flag (and drops state to `IDLE`), `isStopRequested()` is polled by the accessibility service's off-loop polls, `clearStopRequest()` resets it (called on every session entry point and the Play/resume path). Short-term memory lives in `SessionTranscript` (structured plan checklist + rolling `RECENT_OBSERVATION_LOG` of the last ~10 transaction digests + action log; renders the `<ACTIVE_PLAN>`/`<RECENT_OBSERVATION_LOG>` blocks; compaction caps checklist items, digest size, and plan notes). Both reset on session init/reset. `ToolboxManager` is the single tool registry — the overlay drawer buttons and the floating-bubble menu both render from it via `getTool(id)` (no duplicated per-surface wiring). `AgentPolicy` holds persisted `deny <COMMAND>:<path-glob>` rules (JSON in prefs) enforced in `TransactionManager` and re-injected as `<POLICY>` in the boot prompt; no policy rule can weaken the DELETE/MOVE human-confirmation invariant.
- `utils/SystemPromptGenerator.kt` — builds the bootstrap system prompt (`generate`), pulled in by `AgentOrchestrator.buildSystemPrompt`. There is no real chat-app `system` channel; the whole prompt is typed as the first user message, so it drifts on long ReAct loops. Mitigations: an immutable `PRIMARY_RULES` block, `standingReminder` (re-anchors rules on every `TRANSACTION_RESULT`/`TRANSACTION_ERROR`), and `truncateForInjection` (caps oversized feedback and embedded `CodeAssist.md` content, e.g. 12000 chars for project context). The prompt is written opencode-style (`#`-sectioned) with a consequence-aware Guardrails section: every rule states what the app actually does on violation (e.g. mixed DONE is ignored, truncated envelopes are dropped, DELETE/MOVE can't be bypassed) so the model internalizes the protocol instead of treating it as decoration. Keep the tool list in sync with `EnvelopeParser`/`CodeCommand` (currently GLOB, OUTLINE, READ, GREP, PATCH, CREATE, DELETE, MOVE, PLAN, DONE).
...
- `ui/` — `AgentOverlayManager`, `OverlayConfirmationManager`, `BoundingBoxView`, `ScrollZonePickerView`, `TransactionSummaryController`. `AgentOverlayManager` hosts the drag pill (position clamped to the screen and persisted to `CodeAssistPrefs` `OVERLAY_POS_X`/`OVERLAY_POS_Y`) and the drawer, whose tool buttons dispatch through `ToolboxManager.getTool(id)`.
